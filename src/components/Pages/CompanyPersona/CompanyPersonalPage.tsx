...
const CompanyPersonalPage: React.FC = () => {
  ...
  // 3) Quando arriva la lista Redux aggiornata, settiamo editedCompany se manca
  useEffect(() => {
    const companyId = localStorage.getItem("companyId");
  ...
  // 4) Funzione di submit aggiornata senza header Authorization, usiamo cookie per autenticazione
  const handleSubmit = async (data: any) => {
    if (!editedCompany) return;
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/companies/updatePortion/${editedCompany._id}`,
        {
          ...
          credentials: "include", // invio cookie HttpOnly per autenticazione
          body: JSON.stringify(updatedCompany),
        }
      );
      ...
      // Rifacciamo fetch dei dati aggiornati
      const updatedCompanyResponse = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/companies/${editedCompany._id}`,
        {
          credentials: "include",
